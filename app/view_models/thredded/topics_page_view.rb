...
module Thredded
  ...
  class TopicsPageView
    ...
    def initialize user, topics_page_scope
      ...
      @topic_views = @topics_page_scope.with_read_and_follow_states(user)
                       .map do |topic, read_state, follow|
                         TopicView.new topic, read_state, follow, 
                                       Pundit.policy!(user, topic)
                       end
    end
  end
end
